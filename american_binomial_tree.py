...
def valueOptionMatrix(tree, T, r, K, vol, option_type):
    dt = T / N
    u = np.e**(vol*np.sqrt(dt))
    d = np.e**(-vol*np.sqrt(dt))
    ...
    for i in np.arange(rows-1)[::-1]:       
        for j in range(i+1):
            ...
            S = tree[i,j]
            if option_type == "Put":
                matrix[i, j] = np.maximum(option_value, K - S)
            elif option_type == "Call":
                matrix[i, j] = np.maximum(option_value, S - K)
...
tree = buildTree(stock_price_T0, sigma, T, N)
option_worth=valueOptionMatrix(tree, T, r, strike_price, sigma, option_type)
